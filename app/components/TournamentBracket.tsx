...
import { ReactFlow, Node, Edge, useNodesState, useEdgesState, Controls, Background, MiniMap, BackgroundVariant, ConnectionLineType } from '@xyflow/react';
import { cn } from "@/lib/utils";
import { useSport } from '../providers/SportContext';
import { Shield, Trophy, Calendar, Clock, MapPin, Palette, Grid, Sparkles } from "lucide-react";
import { 
  generateBracket, 
  generateLayout, 
...
const backgroundOptions = [
  ...
  { 
    ...
    label: "Gradiente", 
    icon: Palette,
    className: "bg-gradient-to-br from-background via-background-elevated to-background-card",
    pattern: "lines"
  },
  { 
    key: "sport", 
    label: "Esportivo", 
    icon: Sparkles,
    className: "bg-gradient-to-br from-orange-900/20 via-orange-800/10 to-orange-700/5",
    pattern: "cross"
  },
  { 
    key: "dark", 
...
function CustomBackground({ pattern, className }: { pattern: string; className: string }) {
...
function EnhancedMatchNode({ data }: { data: any }) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'live': return 'text-accent-live bg-accent-live/10 border-accent-live/20';
      case 'completed': return 'text-accent-success bg-accent-success/10 border-accent-success/20';
      case 'upcoming': return 'text-accent-secondary bg-accent-secondary/10 border-accent-secondary/20';
      case 'postponed': return 'text-accent-warning bg-accent-warning/10 border-accent-warning/20';
      default: return 'text-text-secondary bg-background-card border-border-default';
    }
  };
            ...
            <div className="flex items-center gap-2">
              {team.score !== undefined && (
                <span className={cn(
                  ...
                  team.isWinner ? "text-accent-primary" : "text-text-secondary"
                )}>
                  {team.score}
                </span>
              )}
...
function HierarchyView({ tournamentData }: { tournamentData: TournamentData }) {
  ...
  const tree = d3.tree().size([height - margin.top - margin.bottom, width - margin.right - margin.left]);
  const treeData = tree(hierarchyData as any);

  return (
    <div className="w-full overflow-auto relative">
      <CustomBackground pattern="dots" className="bg-gradient-to-br from-orange-900/10 via-orange-800/5 to-transparent" />
...
function StatisticsView({ tournamentData }: { tournamentData: TournamentData }) {
  const stats = useMemo(() => calculateTournamentStats(tournamentData), [tournamentData]);
  const winner = useMemo(() => getTournamentWinner(tournamentData), [tournamentData]);

  return (
    <div className="space-y-6 relative">
      <CustomBackground pattern="lines" className="bg-gradient-to-br from-orange-900/5 via-orange-800/3 to-transparent" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 relative z-10">
        <div className="bg-background-card/95 backdrop-blur-sm border border-border-default rounded-lg p-4 hover:shadow-lg transition-all duration-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-text-secondary">Total de Jogos</span>
          </div>
          <span className="text-2xl font-bold text-text-primary">{stats.totalMatches}</span>
        </div>
        ...
        <div className="bg-background-card/95 backdrop-blur-sm border border-border-default rounded-lg p-4 hover:shadow-lg transition-all duration-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-text-secondary">Concluídos</span>
          </div>
          <span className="text-2xl font-bold text-accent-success">{stats.completedMatches}</span>
        </div>
        ...
        <div className="bg-background-card/95 backdrop-blur-sm border border-border-default rounded-lg p-4 hover:shadow-lg transition-all duration-200">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-text-secondary">Ao Vivo</span>
          </div>
          <span className="text-2xl font-bold text-accent-live">{stats.liveMatches}</span>
        </div>
        ...
        <div className="bg-background-card/95 backdrop-blur-sm border border-border-default rounded-lg p-4 hover:shadow-lg transition-all duration-200">
          ...
          <span className="text-2xl font-bold text-accent-secondary">{stats.upcomingMatches}</span>
        </div>
      </div>
      ...
      <div className="bg-background-card/95 backdrop-blur-sm border border-border-default rounded-lg p-4 relative z-10 hover:shadow-lg transition-all duration-200">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-text-secondary">Progresso do Torneio</span>
          <span className="text-sm font-bold text-text-primary">{Math.round(stats.progressPercentage)}%</span>
        </div>
...
export default function TournamentBracket({ teams = mockTeams, sportData }: TournamentBracketProps) {
  ...
  useMemo(() => {
    if (tournamentData.rounds.length > 0) {
      const { nodes: newNodes, edges: newEdges } = generateLayout(tournamentData);
  ...
  const currentBackground = backgroundOptions.find(bg => bg.key === backgroundStyle) || backgroundOptions[0];
        ...
        <div className="h-[600px] border border-border-default rounded-lg overflow-hidden relative">
          ...
          >
            ...
            <Background 
              ...
              size={1}
              color={currentBackground.pattern === 'dots' ? 'rgba(255, 107, 53, 0.1)' : 'rgba(255, 107, 53, 0.05)'}
            />
            <MiniMap 
              nodeColor="#ff6b35"
