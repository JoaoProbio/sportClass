 ...
 *
 * NOTE: When NEXT_PUBLIC_API_BASE is not set in the environment we default to
 * the mainone hosted service so the frontend can communicate with the expected
 * backend without requiring local proxy configuration.
 */
const BASE =
  process.env.NEXT_PUBLIC_API_BASE || "https://mainone-ptsq.onrender.com";
...
function compute404Alternatives(originalPath: string): string[] {
  ...
  if (pathPortion.endsWith("/")) {
    if (pathPortion.length > 1) {
      candidates.push(pathPortion.slice(0, -1));
    }
  } else {
    candidates.push(`${pathPortion}/`);
  }

  // Remove duplicates and the original path if it's present
  const uniqueCandidates = [...new Set(candidates)];
  return uniqueCandidates.filter((c) => c !== pathPortion);
}
...
async function fetch_with_retries(
  ...
  if (DEBUG)
    console.debug("apiClient fetch:", options.method || "GET", url, {
      options,
      retries,
    });
  ...
  try {
    ...
    // If we get a 404, try alternative paths before giving up.
    if (res.status === 404) {
      // 1) Try path-based alternatives (add/remove /api, trailing slash variants, etc.)
      const alternatives = compute404Alternatives(path);
      if (alternatives.length > 0) {
        if (DEBUG)
          console.debug("apiClient 404, trying alternatives:", alternatives);
        for (const altPath of alternatives) {
          try {
            const altUrl = buildUrl(altPath);
            if (DEBUG)
              console.debug("apiClient trying 404 alternative:", altUrl);
            const altRes = await fetch(altUrl, options);
            if (altRes.ok) {
              if (DEBUG)
                console.debug("apiClient 404 alternative success:", altUrl);
              return altRes; // Found a working alternative
            }
          } catch (altErr) {
            // ignore errors on alternatives, we'll continue trying other fallbacks
          }
        }
      }

      // 2) Try host-based alternatives: swap between configured BASE and current origin
      //    This helps when the client is requesting same-origin /api/... but the backend
      //    actually lives at the configured BASE (or vice-versa).
      try {
        const hosts: string[] = [];

        // include explicit BASE if available
        if (BASE) hosts.push(BASE.replace(/\/+$/, ""));

        // include browser origin when running in the browser
        if (typeof window !== "undefined" && window.location?.origin) {
          hosts.push(window.location.origin.replace(/\/+$/, ""));
        }

        // include the url's origin if the built url was absolute
        try {
          if (/^https?:\/\//i.test(url)) {
            const parsedUrl = new URL(url);
            if (parsedUrl.origin)
              hosts.push(parsedUrl.origin.replace(/\/+$/, ""));
          }
        } catch {
          // ignore
        }
        ...
        // compute the pathname+search+hash to reuse across hosts
        let pathPortion = "";
        try {
          const baseForUrl =
            typeof window !== "undefined" && window.location?.origin
              ...
              : BASE || "";
          const parsed = new URL(url, baseForUrl);
          pathPortion = `${parsed.pathname}${parsed.search || ""}${parsed.hash || ""}`;
        } catch {
          // fallback to the raw path if URL parsing fails
          pathPortion = path.startsWith("/") ? path : `/${path}`;
        }

        for (const host of uniqHosts) {
          if (!host) continue;
          const hostNoTrailing = host.replace(/\/+$/, "");
          // candidate 1: host + exact pathPortion
          const candidate1 = `${hostNoTrailing}${pathPortion.startsWith("/") ? pathPortion : `/${pathPortion}`}`;
          // candidate 2: if pathPortion doesn't already start with /api, try adding /api prefix
          const candidate2 = pathPortion.startsWith("/api")
            ? null
            : `${hostNoTrailing}/api${pathPortion.startsWith("/") ? pathPortion : `/${pathPortion}`}`;

          const hostCandidates = candidate2
          ...
          for (const candidate of hostCandidates) {
            try {
              if (DEBUG)
                console.debug("apiClient trying host alternative:", candidate);
              const hostRes = await fetch(candidate, options);
              if (hostRes.ok) {
                if (DEBUG)
                  console.debug(
                    "apiClient host alternative success:",
                    candidate,
                  );
                return hostRes;
              }
            } catch (hostErr) {
              // ignore and try next host/candidate
            }
          }
        }
      } catch (hostFallbackErr) {
        if (DEBUG)
          console.debug(
            "apiClient host-based alternatives failed",
            hostFallbackErr,
          );
      }
    }
    ...
    const isConnectionError = CONNECTION_ERROR_CODES.some((code) =>
      (error.cause?.code || "").includes(code),
    );

    if (isConnectionError && retries > 0) {
      if (DEBUG)
        console.debug("apiClient connection error, retrying...", {
          error,
          retriesLeft: retries - 1,
        });
      await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
      return fetch_with_retries(path, options, retries - 1);
    }

    if (DEBUG) console.error("apiClient fetch failed:", { error });
    // Re-throw fetch errors (e.g. network) as ApiError for consistent handling
    const err: ApiError = new Error(
      error.message || "Network error",
    ) as ApiError;
    err.status = 0; // custom status for network errors
    err.body = null;
    throw err;
  }
}
...
async function request(
  ...
  const options: RequestInit = {
    method,
    headers: buildHeaders(extra, body),
  };
  if (body) {
    options.body =
  ...
  const res = await fetch_with_retries(path, options);
  return parseResponse(res);
}

const api = {
  get: (path: string, extra?: Record<string, string>) =>
    request("GET", path, null, extra),
  post: (path: string, body?: any, extra?: Record<string, string>) =>
    request("POST", path, body, extra),
  put: (path: string, body?: any, extra?: Record<string, string>) =>
    request("PUT", path, body, extra),
  patch: (path: string, body?: any, extra?: Record<string, string>) =>
    request("PATCH", path, body, extra),
  delete: (path: string, body?: any, extra?: Record<string, string>) =>
    request("DELETE", path, body, extra),
  setAuthToken,
  setOnUnauthorized,
};
